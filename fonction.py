...
def tf_idf(list_of_files):
    """
        tf_idf : Calcule le score TF-IDF grâce au deux dictionnaires donnés par les deux fonctions ("tf" et "idf").
        - Entrées : list_of_files = liste de nom de fichier.
        - Sortie : m_tf_idf = une matrice des vecteurs TF-IDF (colonne = fichier, ligne = mot)
                   dic_column = un dictionnaire des fichiers (key = nom du fichier, valeur = index (colonne) des fichiers dans la matrice "m_tf_idf")
                   dic_line = un dictionnaire des mots (key = mots, valeur = index (colonne) des mots dans la matrice"m_tf_idf")
        /!\ la sortie des 3 variables se fait en tuple.
    """
